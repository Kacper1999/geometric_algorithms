from voronoi_diagram.Structures import LineType, Bisection, Point, TaxiCabParabola, Line
        ...
        for j in range(1, i):
            for k in range(j):
                ...
                if(isVoronoiPoint(tab, i, j, k, proposedPoint)):
                    ...
                    scenes.append(Scene([makePointsCollection(foundPoints), makePointsCollection(tab, 'green')], [makeLinesCollection(boundLines, 'purple')]))
        ...
        for j in range(i, len(voronoiPoints)):
            if(isVoronoiLine(voronoiPoints, i, j)):
                ...
                scenes.append(Scene([makePointsCollection(voronoiPoints), makePointsCollection(tab, 'green')],
                  [makeLinesCollectionFromBisection(voronoiLines), makeLinesCollection(boundLines, 'purple')]))
    ...
    for i in range(len(voronoiPoints)):
        ...
        for sec1 in range(1,3):
            for sec2 in range(0,sec1):
                ...
                if(not flag): 
                    if(sec1 !=0 and sec2 != 0): sec3 = 0
                    elif(sec1 !=1 and sec2 !=1): sec3 = 1
                    else: sec3 = 2
                    voronoiLines.append(createVoronoiPolprosta(tab, voronoiPoints[i], closestSectors[sec1], closestSectors[sec2], closestSectors[sec3]))
                    scenes.append(Scene([makePointsCollection(voronoiPoints), makePointsCollection(tab, 'green')],
                    [makeLinesCollectionFromBisection(voronoiLines), makeLinesCollection(boundLines, 'purple')]))
    return voronoiLines
